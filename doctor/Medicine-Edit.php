            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget">
                        ...
                        <?php 
                    $M_ID = isset($_GET['mid']) ? $_GET['mid'] : 0 ; // To GET Medicine ID
                    if($_SERVER['REQUEST_METHOD'] == "POST")  {
                        ...
                        $stmt= $dbh->prepare("UPDATE tblpharmacy SET M_Name = ? , M_Price = ? , Image = ? WHERE ID = ?") ;
                        $stmt->execute(array($M_Name , $M_Price , $image_name ,  $M_ID)) ; 
                    }
    ?>
                          <form action="" method="POST" enctype="multipart/form-data">
                              <?php 
                                    $stmt2 = $dbh->prepare("SELECT * FROM tblpharmacy WHERE ID = ?");
                                    $stmt2->execute(array($M_ID)) ;
                                    $row = $stmt2->fetch() ;
                              ?>
                              <div class="form-group">
                                  <label for="">Enter The Medecine Name: </label>
                                  <input type="text" class="form-control"  name="name" value="<?php echo $row['M_Name'] ; ?>">
                                </div>
                                <div class="form-group">
                                    <label for="">Enter The Medecine Price: </label>
                                    <input type="number" min=0  class="form-control m-5" name="price"  value="<?php echo $row['M_Price'] ; ?>"> 
                                </div>
                                <div class="form-group">
