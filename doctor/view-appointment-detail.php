...
<button class="btn btn-primary waves-effect waves-light w-lg" data-toggle="modal" data-target="#roch">Add Prescription</button></p>  

<div class="modal fade" id="roch" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
     <div class="modal-content">
      <div class="modal-header">
  ...
  <?php 
    $stmt44 = $dbh->prepare("SELECT * FROM tblpharmacy") ; 
    $stmt44->execute();
    $rows44 = $stmt44->fetchAll();
  

    echo '<select name="med" class="form-control">' ; 
          foreach($rows44 as $row44) {
            echo '<option value="' . $row44['ID'] . '">' . $row44['M_Name'] .'</option>' ;
          }
    echo '</select>' ; 
  ?>
  <div class="notes">
    ...
    <input type="hidden" name="patient" value="<?php echo $row->PID; ?>">
...
<?php
if(isset($_POST['save']))  {
    $Prescription_Name = $_POST['med'] ;
    $notes =$_POST['notes'] ;
    $patient = $_POST['patient'] ; 
    $doctor = $_POST['doctor'] ;
    $stmt33= $dbh->prepare("INSERT INTO prescription(Prescription_Name, Notes , PID ,DID ) VALUES (? , ? , ? , ? )") ;
    $stmt33->execute(array($Prescription_Name  , $notes , $patient , $doctor)) ; 
} ?>
